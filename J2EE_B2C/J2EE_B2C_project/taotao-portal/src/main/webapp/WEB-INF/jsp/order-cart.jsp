...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<c:forEach items="${cartList }" var="cart" varStatus="status">
			...
			<input type="hidden" name="orderItems[${status.index}].num" value="${cart.quantity }"/>
			<input type="hidden" name="orderItems[${status.index}].price" value="${cart.price}"/>
			<input type="hidden" name="orderItems[${status.index}].totalFee" value="${cart.price * cart.quantity}"/>
		...
		<div class="goods-item goods-item-extra">
			...
			<div class="goods-msg">
				...
				<div class="p-price">
					...
					<!--增加预售金额显示 end-->
					<span class="ml20"> x${cart.quantity} </span> 
